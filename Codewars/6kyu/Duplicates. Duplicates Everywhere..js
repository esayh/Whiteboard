...
const removeDuplicateIds = (obj) => {
  ...
  const allKeys = Object.keys(obj).sort((a, b) => {return b - a})
  
  for(const itm of allKeys) {
    newObj[itm] = []
    
    for(let i = 0; i < obj[itm].length; i++) {
      if(!newSet.has(obj[itm][i])) {
        
        newObj[itm].push(obj[itm][i])
        
        newSet.add(obj[itm][i])
      }
    }
  }
  return newObj
